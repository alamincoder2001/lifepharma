...
class Page extends CI_Controller {
    ...
    public function genetic() {
        ...
        $data['title'] = "Add Genetic";
        $data['content'] = $this->load->view('Administrator/genetic', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    }

    public function getGenetices() {
        $genetices = $this->db->query("
            select 
                *
            from tbl_genetic
            where status = 'a'
        ...
        echo json_encode($genetices);
    }

    public function addGenetic() {
        $res = new stdClass;
        try {
            $data = json_decode($this->input->raw_input_stream);

            $duplicateName = $this->db->query("select * from tbl_genetic where status = 'a' and name = ? and branch_id = ?", [$data->name, $this->session->userdata('BRANCHid')]);
            if($duplicateName->num_rows() > 0) {
                $res->message = 'This genetic name already exists';
            ...
            $this->db->insert('tbl_genetic', $genetic);
            $res->message = 'Genetic insert successfully';
            $res->success = true;
        } catch (\Exception $e) {
    ...
    public function updateGenetic() {
        $res = new stdClass;
        try {
            $data = json_decode($this->input->raw_input_stream);

            $duplicateName = $this->db->query("select * from tbl_genetic where status = 'a' and name = ? and branch_id = ? and id != ?", [$data->name, $this->session->userdata('BRANCHid'), $data->id]);
            if($duplicateName->num_rows() > 0) {
                $res->message = 'This genetic name already exists';
                echo json_encode($res);
                exit;
            ...
            $this->db->where('id', $data->id)->update(': tbl_companies', $genetic);
            $res->message = 'Genetic update successfully';
            $res->success = true;
        } catch (\Exception $e) {
            $res->message = 'failed '. $e->getMessage();
        }
    ...
    public function deleteGenetic() {
        $res = new stdClass;
        try {
            $data = json_decode($this->input->raw_input_stream);
            
            $checkUsed = $this->db->query("select genetic_id from tbl_product where genetic_id = ? and Product_branchid = ?", [$data->id, $this->session->userdata('BRANCHid')]);
            if($checkUsed->num_rows() > 0) {
                $res->success = false;
                $res->message = 'Could not deleted. This name already used product';
                echo json_encode($res);
            }

            $this->db->set('status', 'd')->where('id', $data->id)->update(': tbl_companies');
            $res->message = "Genetic delete successfully";
            $res->success = true;
        } catch (\Exception $e) {
            $res->message = "Failed ". $e->getMessage();
        }
    ...
    public function company() {
        ...
        $data['content'] = $this->load->view('Administrator/company', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    }
    ...
    public function addCompany() {
        ...
        try {
            ...
            $duplicateName = $this->db->query("select * from tbl_companies where status = 'a' and name = ? and branch_id = ?", [$data->name, $this->session->userdata('BRANCHid')]);
            if($duplicateName->num_rows() > 0) {
            ...
            $res->message = 'Company insert successfully';
            $res->success = true;
        } catch (\Exception $e) {
            $res->message = 'failed '. $e->getMessage();
        }
    ...
    public function updateCompany() {
        ...
        try {
            ...
            $duplicateName = $this->db->query("select * from tbl_companies where status = 'a' and name = ? and branch_id = ? and id != ?", [$data->name, $this->session->userdata('BRANCHid'), $data->id]);
            if($duplicateName->num_rows() > 0) {
                $res->message = 'This company name already exists';
                echo json_encode($res);
                exit;
            ...
            $this->db->where('id', $data->id)->update('tbl_companies', $company);
    ...
    public function deleteCompany() {
        ...
        try {
            ...
            $checkUsed = $this->db->query("select company_id from tbl_product where company_id = ? and Product_branchid = ?", [$data->id, $this->session->userdata('BRANCHid')]);
            if($checkUsed->num_rows() > 0) {
                $res->success = false;
                $res->message = 'Could not deleted. This name already used product';
                echo json_encode($res);
    ...
    public function addBranch()  {
        ...
        try{
            ...
            $id = $this->db->insert_id();
                if(!empty($_FILES)) {
                    $config['upload_path'] = './uploads/brunch';
                    $config['allowed_types'] = 'gif|jpg|png';
                    $imageName = 'brunch-'.$id;
                    $config['file_name'] = $imageName;
                    $this->upload->initialize($config);
                    $this->upload->do_upload('image');
                    $imageName = 'brunch-'.$id . $this->upload->data('file_ext');
                    $this->db->query("update tbl_brunch set Brunch_Image = ? where brunch_id = ?", [$imageName, $id]);
                }
    ...
    public function updateBranch()  {
        ...
        try{
           ...
           // var_dump($branch->branchId);
            ...
            $oldImages = $this->db->query('select * from tbl_brunch where brunch_id = ?', $branch->branchId)->row();
           
                if(!empty($_FILES)) {
                    $path = "uploads/brunch/$oldImages->Brunch_Image";
                    if (file_exists($path)) {
                        unlink($path);
                    }
                    $config['upload_path'] = './uploads/brunch';
                    $config['allowed_types'] = 'gif|jpg|png';
                    $imageName = 'brunch-'.$branch->branchId;
                    $config['file_name'] = $imageName;
                    $this->upload->initialize($config);
                    $this->upload->do_upload('image');
                    $imageName = 'brunch-'.$branch->branchId . $this->upload->data('file_ext');
                    $this->db->query("update tbl_brunch set Brunch_Image = ? where brunch_id = ?", [$imageName, $branch->branchId]);
                }
