    ...
    <div class="row" style="margin-top: 15px;">
        <div class="col-md-8">
            <form class="form-horizontal" @submit.prevent="addDamage">
                ...
                <div class="form-group" :style="{display: dateStock.length > 0 ? '' : 'none'}">
                    <label class="col-sm-6 control-label no-padding-right"> Expire Date </label>
                    <label class="col-sm-1 control-label no-padding-right">:</label>
                    <div class="col-sm-5">
						<v-select v-bind:options="dateStock" label="expire_date" v-model="selectedDateStock" v-on:input="dateOnChange"></v-select>
                    </div>
				</div>
        ...
        methods: {
            async productOnChange() {
                if((this.selectedProduct.Product_SlNo != '' || this.selectedProduct.Product_SlNo != 0)){
                    ...
                    // this.damage.expire_date = this.selectedProduct.expire_date;
                }
                ...
                if ( this.selectedProduct.Product_SlNo != '' || this.selectedProduct.Product_SlNo != 0 ) {
                    ...
                    this.productStock = await axios.post('/get_product_stock', {
                        productId: this.selectedProduct.Product_SlNo,
						expiredDate:  this.selectedDateStock ? this.selectedDateStock.expire_date : null
                    }).then(res => {
            ...
            async dateOnChange() {
                if(this.selectedDateStock != null) {
                    this.damage.expire_date = this.selectedDateStock.expire_date;

                    this.productStock = await axios.post('/get_product_stock', {
                        productId: this.selectedProduct.Product_SlNo || null,
                        expiredDate: this.selectedDateStock.expire_date || null
                    }).then(res => {
                            return res.data[0].current_quantity;
                        })
                } 
            ...
            async editDamage(damage){
                ...
                this.selectedProduct = {
					...
					expired_available: damage.expired_available
                }
				if(this.selectedProduct.expired_available == 1 ) {
					this.selectedDateStock = {
						Product_Name: damage.Product_Name,
						expire_date: damage.damage_productExpire
					}
				} else {
					this.selectedDateStock = null;
				}
                ...
                //     this.dateStock = await axios.post('/get_available_date', {
                //         productId: this.selectedProduct.Product_SlNo || null,
                //         expired_available: this.selectedProduct.expired_available == 1 ? 1 : 0
                //     }).then(res => {
                //         return res.data;
                //     })
                // }
